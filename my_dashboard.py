...
def pie_plot(data, keyword):
    ...
    colors = ["#031cfc", "#24b514", "#d11d1d"]
    for col, color in zip(cols, colors):
        plot_data.append(go.Pie(labels=res.index.to_list(), values=res[col], name=col, marker=dict(colors=[color])))
...
def line_plot(data, keyword):
    ...
    colors = ["#031cfc", "#24b514", "#d11d1d"]
    for col, color in zip(cols, colors):
        plot_data.append(go.Scatter(x=res.index.to_list(), y=res[col], name=col, mode='lines', line=dict(color=color)))
...
def plot_violinplots(data, keyword="Deaths/1M pop"):
    """
    This function returns a figure of the violin plot related to each continent in regards to the keyword.

    Parameters:
        data : dataframe
            The whole dataset.
        keyword : str, optional
            The keyword used to define the data for the violin plot. Default is "Deaths/1M pop".
    ...
    for continent in continents:
        plot_data.append(go.Violin(y=grouped_data.get_group(continent)[keyword], name=continent))
    layout = go.Layout(title=f"Violin plots using {keyword}",
                       xaxis=dict(title="Continents"),
...
def init_figure():
    ...
    return plot_continent_data(data, keyword="New"), plot_top_k_countries(10, "TotalCases"), plot_boxplots(data), \
        pie_plot(data, keyword="TotalCases"), line_plot(data, keyword="TotalCases"), plot_violinplots(data)
...
init_continent_fig, init_k_countries_plot, init_box_fig, init_pie_fig, init_line_fig, init_violin_fig = init_figure()
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
        html.Br(),
        html.H2("ViolinPlot to explain the distribution of the variables", style={"text-align": "center"}),
        html.Br(),
        dcc.Dropdown(id="select_vio_attribute",
                     options=[
                         dict(label="Deaths per 1M population", value='Deaths/1M pop'),
                         dict(label="Tests per 1M population", value='Tests/1M pop')
                     ],
        ...
        dcc.Graph(id="continent_vio_plot", figure=init_box_fig)
    ]),

    html.Div([
        html.Br(),
        html.H2("Corona Cases/Recovered/Deaths by Continent : Pie", style={"text-align": "center"}),
        html.Br(),
        dcc.Dropdown(id="select_pie",
                     options=[
                         dict(label="Today's Data", value="New"),
                         dict(label="Total Data", value="Total")],
                     multi=False,
        ...
        dcc.Graph(id="continent_corona_pie", figure=init_pie_fig)
    ]),

    html.Div([
        html.Br(),
        html.H2("Corona Cases/Recovered/Deaths by Continent : Line", style={"text-align": "center"}),
        html.Br(),
        dcc.Dropdown(id="select_line",
                     options=[
                         dict(label="Today's Data", value="New"),
                         dict(label="Total Data", value="Total")],
                     multi=False,
        ...
        dcc.Graph(id="continent_corona_line", figure=init_pie_fig)
    ])
])
...
@app.callback(
    Output("continent_vio_plot", "figure"),
    Input("select_vio_attribute", "value")
)
def update_continent_vio_plot(value):
    return plot_violinplots(data, keyword=value)

@app.callback(
    Output("continent_corona_pie", "figure"),
    Input("select_pie", "value")
)
def update_continent_corona_pie(value):
    return pie_plot(data, keyword=value)


@app.callback(
    Output("continent_corona_line", "figure"),
    Input("select_line", "value")
)
def update_continent_corona_line(value):
    return line_plot(data, keyword=value)
